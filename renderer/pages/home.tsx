...
export default function HomePage() {
  ...
  const greenByUserRef = useRef<number>(0); 

  useEffect(() => {
    const { tokens: tokensFromURL } = router.query;

    if (tokensFromURL) {
      const decodedTokens = JSON.parse(decodeURIComponent(tokensFromURL as any));
  ...
  return (
    <React.Fragment>
      ...
      <div className='grid grid-col-1 text-2xl w-full text-center'>
        <LiveStreamComponent greenByUserRef={greenByUserRef} />
        <input
          onChange={(e) => {
            greenByUserRef.current = parseInt(e.target.value);
            console.log(greenByUserRef.current,"--L",parseInt(e.target.value));
          }}
          type="range"
